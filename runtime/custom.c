                            ...
                            const struct custom_operations * ops)
{
  ...
  l->ops = ops;
  struct custom_operations_list * prev = atomic_load_relaxed(table);
  do {
    l->next = prev;
  } while (!atomic_compare_exchange_weak(table, &prev, l));
  /* success implies a release ordering */
}
...
{
  struct custom_operations_list * l;
  for (l = atomic_load_acq(&custom_ops_table); l != NULL; l = l->next)
...
struct custom_operations * caml_final_custom_operations(final_fun fn)
{
  ...
  for (l = atomic_load_acq(&custom_ops_final_table); l != NULL; l = l->next)
